...
class FEASolver:
    ...
    def compute_sensitivity(self, uh):
        ...
        # ==========================================================
        # 🚀 终极通用修复：使用有限元积分法获取绝对物理体积
        # 这种写法兼容 FEniCSx 的任何版本，绝对不会报错！
        # ==========================================================
        V_dg = fem.functionspace(self.domain, ("DG", 0))

        # 1. 构造求单元体积的变分方程：\int 1.0 * v dx (v是DG0测试函数)
        v_test = ufl.TestFunction(V_dg)
        vol_form = fem.form(fem.Constant(self.domain, ScalarType(1.0)) * v_test * ufl.dx)

        # 2. 组装向量，得到的数组就是每个六面体网格的精确体积
        cell_vols = fem.assemble_vector(vol_form).array

        # 3. 提取均值并包装成普通数学常数 (彻底骗过 UFL 解析引擎)
        h_vol = fem.Constant(self.domain, ScalarType(np.mean(cell_vols)))
        # ==========================================================

        # 巧妙复用 Expression 方法免除 assemble_vector 组装时的底层版本阻挠
